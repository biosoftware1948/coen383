...
static Job *getNextJob(
    ...
    if (j1 != NULL && j1->arrival_time <= cpu->global_time) return getFrontQueueElement(p1);
    if (j2 != NULL && j2->arrival_time <= cpu->global_time) return getFrontQueueElement(p2);
    if (j3 != NULL && j3->arrival_time <= cpu->global_time) return getFrontQueueElement(p3);
    if (j4 != NULL && j4->arrival_time <= cpu->global_time) return getFrontQueueElement(p4);
    ...
    if (j1 != NULL && j1->arrival_time == min_arrival) return getFrontQueueElement(p1);
    if (j2 != NULL && j2->arrival_time == min_arrival) return getFrontQueueElement(p2);
    if (j3 != NULL && j3->arrival_time == min_arrival) return getFrontQueueElement(p3);
    if (j4 != NULL && j4->arrival_time == min_arrival) return getFrontQueueElement(p4);
    assert(0);
    return NULL;
}
