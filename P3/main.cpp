...
int main(int argc, char* argv[]) {
	...
	Seller* H0 = new Seller(auditorium, "H0", N, 0);
    H0->StartSelling();
    tids[0] = H0->sellerThread;
	// create M ticket sellers
	for(int i = 1; i < 4; i++) {
		Seller* M = new Seller(auditorium, "M" + std::to_string(i), N, i);
        M->StartSelling();
		tids[i] = M->sellerThread;
	}
	// create  L ticket sellers
	for(int i = 4; i < 10; i++) {
		Seller* L = new Seller(auditorium, "L" + std::to_string(i), N, i);
        L->StartSelling();
