...
bool timeLeftInHour() {
    return timer < MAXIMUM_RUN_TIME;
}

//The constructor also creates the objects thread.
Seller::Seller(std::string auditorium[10][10], std::string type, int N, int id) {
    //generate buyers
	for(int i = 0; i < N; i++) {
		Buyer b;
		createBuyer(&b, i);
...
bool Seller::buyerReady(int timer, int next_buy_time) {
    if(!this->buyerQueue.empty()) {
        if (timer >= next_buy_time ) {
            if (timer >= buyerQueue.top().arrived){
                return true;
            }
        }
    }
